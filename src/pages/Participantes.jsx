...
const histBase = player.puntajeHistoricoBase ?? 0;
  ...
  const rankingLugares = useMemo(() => {
    ...
    return items;
  }, [eventos, player.nombre]);

  const maxLugarCount = rankingLugares[0]?.count || 1;
// ===============================
...
.map(([temp, pos]) => (
      ...
      <section className="playerTables">
        ...
        <div className="tablesGrid">
          ...
          <div className="tableCard">
            <div className="tableTitle">
              🏠 Lugares favoritos de <span style={{ color: player.color }}>{player.nombre}</span>
            </div>
            <div className="tableSubtitle">Los lugares donde mas se junta.</div>

            {rankingLugares.length === 0 ? (
              <div style={{ opacity: 0.7, fontSize: "12px" }}>
                Sin lugares registrados todavia.
              </div>
            ) : (
              <div className="miniTableScroll">
                <table className="miniTable">
                  <thead>
                    <tr>
                      <th>Lugar</th>
                  ...
                  <tbody>
                    {rankingLugares.map((r) => {
                      const pct = Math.round((r.count / maxLugarCount) * 100);
                      return (
                        <tr key={r.key}>
                          <td className="miniName">{r.label}</td>
                          <td className="miniNum">{r.count}</td>
                          <td className="miniBarCell">
                            <div className="miniBarTrack">
                              <div
